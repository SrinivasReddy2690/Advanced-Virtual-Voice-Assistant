...
def brightness_level(query):
    ...
    # Regular expression to match the brightness value
    pattern = r'\bchange brightness to (\d+|[\w-]+)\b'
    ...
    if match:
        try:
            brightness = match.group(1)  # Extracting the brightness value
            brightness_value = int(brightness) if brightness.isdigit() else None  # Convert to integer if possible
            c = wmi.WMI(namespace='wmi')
            methods = c.WmiMonitorBrightnessMethods()[0]
            if brightness_value is not None:
                methods.WmiSetBrightness(brightness_value, 0)
                print("Brightness changed successfully to", brightness_value)
            elif brightness.lower() in brightness_levels:
                brightness_value = brightness_levels[brightness.lower()]  # Get the corresponding brightness value
                methods.WmiSetBrightness(brightness_value, 0)
                print("Brightness changed successfully to", brightness_value)
            else:
                print("Invalid brightness level. Please provide a valid level.")
        except Exception as e:
            print("An error occurred:", e)
    else:
        print("Invalid query. Please provide a valid command.")
